...
# 2. Métodos HTTP
# ===========================================
awk -F\" '{print $2}' "$LOG_FILE" | awk '{print $1}' | sort | uniq -c > "$OUTPUT_DIR/2_http_methods.txt"
...
cat "$OUTPUT_DIR/3_dangerous_methods.txt" >> "$REPORT"

echo -e "\nURLs mais acessadas:" >> "$REPORT"
cat "$OUTPUT_DIR/4_urls.txt" >> "$REPORT"
